...
class ViewTest(BaseTest):
    ...
    def check_write_post(self, extra={}, is_anonymous=False):
        ...
        self.check_contrib_messages(response,
                                    'Message successfully sent.')  # no such check for the following posts, one is enough
    ...
    def test_reply_formatters(self):
        ...
        self.assertContains(response,
                            'bar _ this is my body</textarea>')  # POSTMAN_QUICKREPLY_QUOTE_BODY setting is not involved
    ...
    def test_reply_post_filters(self):
        ...
        response = self.client.post(reverse('postman:reply_with_user_filter_no_reason', args=[pk]), data,
                                    HTTP_REFERER=url)
        self.assertFormError(response, 'form', 'recipients', "Some usernames are rejected: bar, baz.")
        ...
        response = self.client.post(reverse('postman:reply_with_user_filter_exception', args=[pk]), data,
                                    HTTP_REFERER=url)
        self.assertFormError(response, 'form', 'recipients', ['first good reason', "anyway, I don't like bar"])
        ...
        response = self.client.post(reverse('postman:reply_with_exch_filter_no_reason', args=[pk]), data,
                                    HTTP_REFERER=url)
        self.assertFormError(response, 'form', 'recipients', "Writing to some users is not possible: bar, baz.")
        ...
        response = self.client.post(reverse('postman:reply_with_exch_filter_exception', args=[pk]), data,
                                    HTTP_REFERER=url)
        self.assertFormError(response, 'form', 'recipients', ['first good reason', "anyway, I don't like bar"])
    ...
    def test_view_conversation_authentication(self):
        ...
        self.check_status(Message.objects.get(pk=m1.pk), status=STATUS_ACCEPTED, is_new=False, is_replied=True,
                          thread=m1)
        # anonymous
        response = self.client.get(url)
        self.assertRedirects(response, "{0}?{1}={2}".format(settings.LOGIN_URL, REDIRECT_FIELD_NAME, url))
    ...
    def check_update(self, view_name, success_msg, field_bit, pk, field_value=None):
        ...
        response = self.client.post(url, data, HTTP_REFERER=redirect_url,
                                    follow=True)  # 'follow' to access contrib messages
        ...
        # query string has highest precedence
        m1.save()
        response = self.client.post(url_with_success_url + '?next=' + redirect_url, data,
    ...
    def check_update_conversation(self, view_name, root_msg, field_bit, field_value=None):
        ...
        sender_kw = 'sender_{0}'.format(field_bit)
        recipient_kw = 'recipient_{0}'.format(field_bit)
        self.check_status(Message.objects.get(pk=pk), status=STATUS_ACCEPTED, is_new=False, is_replied=True,
                          thread=root_msg, **{sender_kw: field_value})
        self.check_status(Message.objects.get(pk=pk + 1), status=STATUS_ACCEPTED, parent=root_msg, thread=root_msg,
    ...
    def check_read_conversation(self, view_name, pk, field_value=True):
        ...
        self.check_status(Message.objects.get(pk=pk + 2), status=STATUS_ACCEPTED, is_new=not field_value, parent=m2,
                          thread=m1)
        ...
        m2 = Message.objects.get(pk=pk + 1)
        self.check_status(m2, status=STATUS_ACCEPTED, parent=m1, is_replied=True, thread=m1)
        self.check_status(Message.objects.get(pk=pk + 2), status=STATUS_ACCEPTED, is_new=field_value, parent=m2,
                          thread=m1)
        # missing payload
        response = self.client.post(url)
        self.assertRedirects(response, reverse('postman:inbox'))
...
class MessageManagerTest(BaseTest):
    ...
    def test(self):
        ...
        # by messages
        self.assertQuerysetEqual(Message.objects.sent(self.user1, option=OPTION_MESSAGES),
                                 [m7.pk, m6.pk, m5.pk, m3.pk, m2.pk, m1.pk], transform=pk)
        self.assertQuerysetEqual(Message.objects.sent(self.user2, option=OPTION_MESSAGES),
                                 [m10.pk, m9.pk, m8.pk, m4.pk], transform=pk)
        self.assertQuerysetEqual(Message.objects.inbox(self.user1, option=OPTION_MESSAGES), [m8.pk, m4.pk],
                                 transform=pk)
        self.assertQuerysetEqual(Message.objects.inbox(self.user2, option=OPTION_MESSAGES),
                                 [m7.pk, m6.pk, m5.pk, m3.pk], transform=pk)
        ...
        # by conversations
        self.assertQuerysetEqual(Message.objects.sent(self.user1),
                                 [(m7.pk, 0), (m6.pk, 0), (m5.pk, 2), (m2.pk, 0), (m1.pk, 0)], transform=pk_cnt)
        self.assertQuerysetEqual(Message.objects.sent(self.user2), [(m10.pk, 0), (m9.pk, 0), (m8.pk, 0), (m4.pk, 1)],
                                 transform=pk_cnt)
        self.assertQuerysetEqual(Message.objects.inbox(self.user1), [(m8.pk, 0), (m4.pk, 1)], transform=pk_cnt)
        self.assertQuerysetEqual(Message.objects.inbox(self.user2), [(m7.pk, 0), (m6.pk, 0), (m5.pk, 2)],
                                 transform=pk_cnt)

        self.assertQuerysetEqual(Message.objects.thread(self.user1, Q(thread=m3.pk)), [m3.pk, m4.pk, m5.pk],
                                 transform=pk)
        self.assertQuerysetEqual(Message.objects.thread(self.user1, Q(pk=m4.pk)), [m4.pk], transform=pk)
        self.assertQuerysetEqual(Message.objects.thread(self.user2, Q(thread=m3.pk)), [m3.pk, m4.pk, m5.pk],
                                 transform=pk)
        ...
        # by messages
        self.assertQuerysetEqual(Message.objects.archives(self.user1, option=OPTION_MESSAGES), [m6.pk, m1.pk],
                                 transform=pk)
        self.assertQuerysetEqual(Message.objects.archives(self.user2, option=OPTION_MESSAGES), [m10.pk, m6.pk],
                                 transform=pk)
        self.assertQuerysetEqual(Message.objects.trash(self.user1, option=OPTION_MESSAGES), [m8.pk, m3.pk, m2.pk],
                                 transform=pk)
        self.assertQuerysetEqual(Message.objects.trash(self.user2, option=OPTION_MESSAGES), [m9.pk, m7.pk, m4.pk],
                                 transform=pk)
        ...
        self.assertQuerysetEqual(Message.objects.inbox(self.user2, option=OPTION_MESSAGES), [m5.pk, m3.pk],
                                 transform=pk)
        # by conversations
...
class MessageTest(BaseTest):
    ...
    def test_moderation_from_accepted(self):
        ...
        date_in_past = now() - timedelta(days=2)  # any value, just to avoid now()
        msg = Message.objects.create(subject='s', moderation_status=STATUS_ACCEPTED,
                                     moderation_date=date_in_past, moderation_by=self.user1,
                                     recipient_deleted_at=date_in_past)
    ...
    def test_notification_rejection_visitor(self):
        ...
        m = Message.objects.create(subject='s', moderation_status=STATUS_REJECTED, email=self.email,
    ...
    def test_notification_rejection_user(self):
        ...
        m = Message.objects.create(subject='s', moderation_status=STATUS_REJECTED, sender=self.user1,
                                   recipient=self.user2)
    ...
    def test_notification_acceptance_user(self):
        ...
        m = Message.objects.create(subject='s', moderation_status=STATUS_ACCEPTED, sender=self.user1,
                                   recipient=self.user2)
    ...
    def test_notification_acceptance_reply(self):
        ...
        p = Message.objects.create(subject='s', moderation_status=STATUS_ACCEPTED, sender=self.user2,
...
class FiltersTest(BaseTest):
    ...
    def check_or_me(self, x, value, user=None, m=None):
        t = Template(
            "{% load postman_tags %}{{ " + x + "|or_me:user }}")  # do not load i18n to be able to check the untranslated pattern
...
class TagsTest(BaseTest):
    ...
    def test_order_by(self):
        ...
        self.check_order_by('subject', [('foo', 'bar'), (ORDER_BY_KEY, 's'), ('baz', 'qux')],
...
class UtilsTest(BaseTest):
    ...
    def check_email(self, message_template_name, recipient_list, object, action, site, parts=None):
        ...
        if not parts:
            self.assertRaises(TemplateDoesNotExist, email, subject_template, message_template_name, recipient_list,
                              object, action, site)
        else:
    ...
    def test_email(self):
        ...
        self.check_email('email_html_and_empty_txt', recipient_list, m, action, site,
                         ('Html and empty Text\n', '<div>Html and empty Text</div>\n'))
        self.check_email('email_html_and_txt', recipient_list, m, action, site,
